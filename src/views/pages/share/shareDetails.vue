  ...
  <div class="ts-container has-vertically-spaced-large">
      ...
      <div class="ts-grid">
        ...
        <div class="column is-fluid">
          ...
          <div class="ts-grid thumbnail-grid">
            ...
            >
              <div
                class="ts-image is-middle-aligned thumbnail"
          ...
          <div class="ts-content is-vertically-padded">
            <div class="ts-wrap is-center-aligned">
              <button
                class="ts-button is-start-icon"
                :class="{ 'is-outlined': !isCollected }"
                @click="toggleCollection"
              >
                <span
                  class="ts-icon is-star-icon"
                  :class="{ 'is-filled': isCollected }"
                ></span>
                {{ collectionCount }}
              </button>
              <button
                class="ts-button is-start-icon"
                :class="{ 'is-outlined': !isLiked }"
                @click="toggleLike"
              >
                <span
                  class="ts-icon is-heart-icon"
                  :class="{ 'is-filled': isLiked }"
                ></span>
                {{ likeCount }}
              </button>
            </div>
              ...
              <div v-if="productTags.length" class="product-tags-container">
                ...
                >
                  <div class="product-card-content">
                    ...
                    <div
                      v-if="productTag.categoryId && productTag.subcategoryId"
                    >
                      <RouterLink
                        :to="{
                          name: 'shop-link',
                          params: {
                            categoryId: productTag.categoryId,
                            subcategoryId: productTag.subcategoryId,
                          },
                        }"
                        @click.native="
                          filterProductsBySubcategory(
                            productTag.subcategoryId,
                            productTag.categoryId
                          )
                        "
                      >
                        {{ productTag.categoryName }} -
                        {{ productTag.subcategoryName }}
                      </RouterLink>
                    </div>
                    <div v-else>
                      {{ productTag.categoryName }} -
    ...
    <div class="mobile-layout" v-else>
      ...
      <div class="post-details">
        ...
        <h4 class="ts-header">{{ post.postTitle }}</h4>
        <p>
          <i class="ts-icon is-clock-icon"></i> {{ formatDate(post.createdAt) }}
        </p>
...
onMounted(() => {
  ...
  axiosapi
    .get(`/collections/${userStore.userId}/${postId}`)
    .then((response) => {
  ...
  axiosapi
    .get(`/likes/${userStore.userId}/${postId}`)
    .then((response) => {
      isLiked.value = true;
    })
    .catch((error) => {
